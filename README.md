...
- **C-18 was declared red, has only ever been observed green, and is now an
  ...
  specification the row tests rather than something applied." The measurement was
  right and the inference from it was wrong: the CNI is still kindnet with no
  Calico, Cilium or Weave, but kindnetd `v20260528-9350166c` runs a
  ...
  `yadgarhq/docs` ledger 614, is WITHDRAWN, so the deadline it carried is REMOVED
  rather than re-dated and `smoke.yaml` no longer excuses the step with
  `continue-on-error` (ADR-0687).
